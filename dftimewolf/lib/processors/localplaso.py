...
class LocalPlasoProcessor(module.BaseModule):
  ...
  def _DockerRun(self,
                 ...
                 command: str,
                 plaso_input_path: str,
                 plaso_output_path: str):
    volumes = {
      file_container_path: {'bind': plaso_input_path, 'mode': 'ro'},
      self._output_path: {'bind': plaso_output_path, 'mode': 'rw'}
    }
    client = docker.from_env()
    client.containers.run("log2timeline/plaso:latest",
                          volumes=volumes,
                          command=command)
  ...
  def SetUp(self, timezone: Optional[str]=None) -> None:  # pylint: disable=arguments-differ
    ...
    if not self._DeterminePlasoPath():
      if self._CheckDockerImage():
        ...
        self.ModuleError('Could not run log2timeline.py from PATH or a local '
                         'Docker image. To fix: \n'
  ...
  def Process(self) -> None:
    ...
    for file_container in self.state.GetContainers(containers.File, pop=True):
      ...
      if self._use_docker:
        ...
        plaso_input_dir = file_container.path
      ...
      plaso_output_path = os.path.join(
          plaso_output_dir, plaso_output_file)
      cmd.extend([plaso_output_path, plaso_input_dir])

      plaso_storage_file_path = os.path.join(self._output_path,
                                             plaso_output_file)
